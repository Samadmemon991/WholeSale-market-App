...
.controller('RegisterCtrl', function($scope,$http,$ionicPopup,$state) {
  $scope.register = function(user){
    if(user.password != user.cpassword){
      $ionicPopup.show({
          title: "Error",
          subTitle: 'Make sure password and confirm password are same',
          buttons: [
            { text: 'Continue' }]
      });
    }else{
      $scope.code = {};
      $ionicPopup.show({
          title: "Confirmation of Phone",
          subTitle: 'A confirmation message will be sent to '+user.phone,
          buttons: [
            { text: 'Change Number' },
            {
              text: 'Send SMS',
              ...
              onTap: function(e) {
                ...
                $http.get("http://localhost/warehouse/sms.php?number="+user.phone).then(function(response){
                  $ionicPopup.show({
                    template: '<input type = "text" ng-model="code.code">',
                    ...
                    buttons: [
                      {
                        ...
                        type: 'button-positive',
                        onTap: function(e) {
                          alert($scope.code.code)
                          if($scope.code.code == response.data.code){
                            alert("Verified");
                          }
                        }
                    }]
